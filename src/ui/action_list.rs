...
pub fn render(mut idState: IdState, ui_cell: &mut UiCell, ref client: &Client, state: &State) -> IdState {
    ...
    let parent_id = idState.last(&canvases, "client_canvases").unwrap();

    let ids = idState.ids.action_lists.to_vec();
    let id = idState.next(&ids, "action_lists").unwrap();

    let (mut events, scrollbar) = widget::ListSelect::multiple(client.history_item_amount())
         .flow_down()
         .item_size(50.0)
         .scrollbar_next_to()
         .w_h(250.0, 700.0)
    ...
    let mut selections = client.selections.clone();
